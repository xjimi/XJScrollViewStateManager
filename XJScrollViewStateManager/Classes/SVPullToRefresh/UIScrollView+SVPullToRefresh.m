...
- (void)layoutSubviews
{
    ...
    if(hasCustomView) {
        ...
        NSDictionary *attr_title = @{ NSFontAttributeName : self.titleLabel.font };
        CGSize rect_title = CGSizeMake(labelMaxWidth, self.titleLabel.font.lineHeight);
        CGSize titleSize = [self.titleLabel.text boundingRectWithSize:rect_title
                                                            options:opts
                                                         attributes:attr_title
                                                            context:nil].size;

        NSDictionary *attr_subtitle = @{ NSFontAttributeName : self.subtitleLabel.font };
        CGSize rect_subtitle = CGSizeMake(labelMaxWidth,self.subtitleLabel.font.lineHeight);
        CGSize subtitleSize = [self.subtitleLabel.text boundingRectWithSize:rect_subtitle
                                                                    options:opts
                                                                 attributes:attr_subtitle
                                                                    context:nil].size;
...
- (void)setScrollViewContentInset:(UIEdgeInsets)contentInset
{
    ...
    [self setScrollViewContentInset:contentInset contentOffset:contentOffset];
}
                    ...
                    contentOffset:(CGPoint)contentOffset
{
    ...
    [UIView animateWithDuration:0 animations:^{
        ...
        self.scrollView.contentOffset = coffset;
    } completion:NULL];
}
...
@interface SVPullToRefreshArrow ()
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        UIImage *image = [XJScrollViewStateBundleResource imageNamed:@"PullToRefreshArrow"];
